...
const UserTable = ({ className, setLive }) => {
  ...
  const [size, setSize] = useState(6); // ✅ NEW
  ...
  const { data, isLoading: liveLoading } = useLiveContestants(page, size, order);
  ...
  const handleVote = useCallback((participationId, seasonId, votes) => {
    vote(
  ...
  const sizeOptions = total
    ? [5, 10, 20, 50, 100].filter((num) => num <= total)
    : [5, 10, 20];
    ...
    <div className="absolute top-full mt-1 right-0 sm:left-0 min-w-[70px] bg-[#141821] border border-gray-700 rounded-lg shadow-xl hidden group-hover:block z-50 overflow-hidden">
      {sizeOptions.map((num) => (
        <div
          key={num}
          onClick={() => setSize(num)}
          className="px-4 py-3 sm:py-2 text-sm sm:text-xs text-center sm:text-left text-gray-400 hover:bg-blue-500/10 hover:text-blue-400 cursor-pointer transition-colors"
        >
          {num}
        </div>
      ))}
